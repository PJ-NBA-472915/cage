...
"""
            ...
            subprocess.run(["git", "commit", "-m", f"Initial commit for {test_name}"], cwd=repo_path, check=True)
...
def main():
    parser = argparse.ArgumentParser(description="Manage isolated test repositories for Cage platform")
    parser.add_argument("command", choices=["create", "list", "start", "cleanup", "cleanup-all"], 
    ...
    if args.command == "create":
        ...
        if repo_path:
            ...
            print(f"🚀 To start Cage service: python manage-isolated-tests.py start --name {args.name}")
        ...
        if not repo_path.exists():
            ...
            print("Create it first with: python manage-isolated-tests.py create --name <name>")
            sys.exit(1)
        ...
        if input("⚠️  This will remove ALL test repositories. Continue? (y/N): ").lower() == 'y':
            manager.cleanup_all()
        else:
