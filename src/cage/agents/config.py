...
class AgentConfigManager:
    ...
    def __init__(self, config_dir: Optional[Path] = None, logger: Optional[logging.Logger] = None):
        ...
        self.logger.info(f"Agent configuration manager initialized with config dir: {self.config_dir}")
    ...
    def load_config_from_env(self, agent_name: str) -> Optional[AgentConfig]:
        ...
        if not all([role, goal, backstory]):
            self.logger.warning(f"Incomplete environment configuration for agent {agent_name}")
            return None
        
        # Parse boolean values
        verbose = os.getenv(f"{prefix}VERBOSE", "true").lower() == "true"
        allow_delegation = os.getenv(f"{prefix}ALLOW_DELEGATION", "false").lower() == "true"
    ...
    def get_config(self, agent_name: str, source: str = "default") -> Optional[AgentConfig]:
        """
    ...
    def create_config_template(self, agent_name: str, config_file: Optional[Path] = None) -> bool:
        """
