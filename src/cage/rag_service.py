...
class RAGService:
    ...
    async def index_file(
        self, file_path: str, content: str, commit_sha: str, branch: str = "main"
    ) -> list[str]:
                ...
                if self.redis_client:
                    try:
                        ...
                        await self.redis_client.setex(
                            cache_key, 86400, chunk
                        )  # Cache for 24 hours
                    except Exception as e:
                        logger.warning(
                            f"Failed to cache chunk content for {cache_key}: {e}"
                        )
        ...
        try:
            ...
            async with self.db_pool.acquire() as conn:
                ...
                if row:
                    ...
                    if full_path.exists():
                        ...
                        if chunk_id < len(chunks):
                            ...
                            if self.redis_client:
                                try:
                                    await self.redis_client.setex(
                                        cache_key, 3600, chunk_content
                                    )  # Cache for 1 hour
                                except Exception as e:
                                    logger.warning(
                                        f"Failed to cache content for {cache_key}: {e}"
                                    )

                            return chunk_content
                        else:
                            logger.warning(
                                f"Chunk {chunk_id} not found in file {file_path}"
                            )
                    ...
                    logger.warning(
                        f"No metadata found for blob {blob_sha} chunk {chunk_id}"
                    )
    ...
    async def reindex_repository(
        self, repo_path: Path, scope: str = "all"
    ) -> dict[str, Any]:
        """Reindex repository content."""
        try:
            logger.info(
                f"DEBUG: Starting reindex of repository at {repo_path} with scope {scope}"
            )
            ...
            logger.info(
                f"Starting reindex of repository at {repo_path} with scope {scope}"
            )
            ...
            if scope in ["all", "repo"]:
                    ...
                    if file_path.is_file():
                        ...
                        if not self._should_skip_file(file_path):
                            ...
                            try:
                                ...
                                logger.info(
                                    f"File content length: {len(content)} characters"
                                )
                                ...
                                logger.info(
                                    f"Successfully indexed {file_path} with {len(chunks)} chunks"
                                )
                            except Exception as e:
                ...
                logger.info(
                    f"File scan complete: {files_found} files found, {files_skipped} skipped, {indexed_files} indexed"
                )
                ...
                if tasks_path.exists():
                    for task_file in tasks_path.glob("*.json"):
                        try:
                            ...
                            logger.warning(
                                f"Failed to index task file {task_file}: {e}"
                            )
