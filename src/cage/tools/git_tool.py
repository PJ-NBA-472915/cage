...
class GitTool:
    ...
    def validate_commit_message(self, message: str) -> tuple[bool, str]:
        ...
        if not re.match(conventional_pattern, first_line):
            self.logger.warning(
                f"Commit message doesn't follow conventional format: {first_line}"
            )
        ...
        if first_line.islower() and not first_line.startswith(
            (
                "feat",
                "fix",
                "docs",
    ...
    def format_commit_message(self, message: str, task_id: str = None) -> str:
        ...
        if task_id and not message.startswith(
            (
                "feat(",
                "fix(",
                "chore(",
            ...
            parts = task_id.split("-")
            if (
                len(parts) >= 4 and parts[0].isdigit() and len(parts[0]) == 4
            ):  # Year format
    ...
    def _run_git_command(
        self, command: list[str], cwd: Path = None, env: dict[str, str] = None
    ) -> GitOperationResult:
        ...
        try:
            ...
            return GitOperationResult(
                success=False, error=str(e), data={"exception": str(e)}
            )
    ...
    def init_repo(self) -> GitOperationResult:
        ...
        if self.is_git_repo():
            return GitOperationResult(
                success=False, error="Repository already initialized"
            )
    ...
    def commit(
        self, message: str, author: str = None, task_id: str = None
    ) -> GitOperationResult:
        ...
        if not is_valid:
            return GitOperationResult(
                success=False, error=f"Invalid commit message: {validation_error}"
            )
        ...
        if not staged_files:
            ...
            if unstaged_files or untracked_files:
                ...
                return GitOperationResult(
                    success=False, error="No changes detected in the working tree"
                )
        ...
        if result.success:
            ...
            if commit_info.success:
                ...
                self.logger.info(
                    f"Created commit: {commit_info.data.get('sha', 'unknown')[:8]}"
                )
    ...
    def get_commit_info(self, ref: str = "HEAD") -> GitOperationResult:
        ...
        if len(parts) < 6:
            return GitOperationResult(
                success=False, error="Failed to parse commit information"
            )

        commit_sha, title, author_name, author_email, date_str, timestamp = parts

        # Get file statistics
        stats_result = self._run_git_command(["show", "--stat", "--format=", sha])
    ...
    def get_branches(self) -> GitOperationResult:
        ...
        return GitOperationResult(
            success=True, output=result.output, data={"branches": branches}
        )
    ...
    def get_commit_history(self, limit: int = 10) -> GitOperationResult:
        ...
        return GitOperationResult(success=True, data={"commits": commits})
    ...
    def get_diff(self, branch1: str, branch2: str = None) -> GitOperationResult:
        ...
        try:
            ...
            self.logger.error(
                f"Error getting diff between {branch1} and {branch2}: {e}"
            )
