...
class PathValidator:
    ...
    def _validate_repo_root(self):
        ...
        try:
            ...
            if str(self.repo_root) != "/work/repo":
                raise ValueError(
                    f"Repository root must be /work/repo or a subdirectory, got: {self.repo_root}"
                )
    ...
    def normalize_path(self, path: str) -> Path:
        ...
        if ".." in normalized.parts or normalized.is_absolute():
            raise ValueError(
                f"Invalid path: {path} - contains directory traversal or absolute path"
            )
        ...
        if any(part.startswith(".") for part in normalized.parts):
            ...
            if not (len(normalized.parts) >= 1 and normalized.parts[0] == ".cage"):
                raise ValueError(
                    f"Invalid path: {path} - access to hidden files not allowed"
                )
        ...
        if not str(absolute_path).startswith("/work/repo/") and not (
            "/tmp" in str(absolute_path) or "/var/folders" in str(absolute_path)
        ):
            raise ValueError(
                f"Path {path} resolved outside allowed container directory"
            )

        return absolute_path

    def is_allowed_extension(
        self, path: str, allowed_extensions: Optional[list[str]] = None
    ) -> bool:
        """
...
class LinePatchValidator:
    ...
    def apply_line_patch(content: str, operations: list[dict[str, Any]]) -> str:
        ...
        sorted_ops = sorted(
            operations, key=lambda x: x.get("line_number", 0), reverse=True
        )
